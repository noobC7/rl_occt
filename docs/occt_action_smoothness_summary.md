...
下面这些属于当前已经明确、但尚未全部实现的方案。

### 7.1 critic：Phase-aware centralized critic

#### 目标

当前 reward 在不同阶段本身不同，因此 critic 需要显式知道每个 agent 当前对应的阶段信息，以降低 critic loss。

#### 方案

- critic 与 actor 输入分离；
- actor 使用去掉 `hinge_status` 的 observation；
- critic 保持 centralized，可看到所有 agent 的 observation；
- 对 critic 额外输入每个 agent 的 `target_hinge_status`；
- 进一步可做：
  - 共享 trunk
  - `platoon_value_head`
  - `hinge_value_head`
  - 由阶段信息决定哪一头输出，或做 phase-aware blending
...
- `False`：沿用当前单头 centralized critic；
- `True`：critic 使用双头 value 结构，并利用额外输入的 `target_hinge_status` 选择/调制输出。

### 7.2 actor：Retentive single-head actor with LSTM

#### 目标

通过显式编码历史信息，提高对他车运动趋势和 hinge 目标变化趋势的理解，从而提高铰接效率，同时保持横纵动作耦合。

#### 设计原则

- actor 不分多个动作头；
- acceleration 和 steering 仍由同一个最终动作头联合输出；
- 历史建模只放在真正需要趋势信息的部分；
- 自车当前状态不强行送入 LSTM。
...
- `past_relative_hinge_info.get_latest(step)[:, agent_index, agent_index, 0, :]`

也就是：

- 仅取第一个 hinge 点；
- 保留其过去 `n_observed_steps` 帧的历史。

##### 他车动态组，使用历史

保留过去 `n_observed_steps` 帧的：
...
- 自车动态组：小 MLP 编码
- hinge 历史组：逐帧小 MLP + LSTM
- 他车历史组：逐帧共享 MLP + LSTM
- 三路特征融合
- 单一动作头输出加速度和前轮转角的联合策略
...
- `False`：沿用当前平铺 MLP actor
- `True`：使用上述带历史编码的单头 LSTM actor

---

## 8. 当前推荐的实验路线

在不改变 reward 定义、保持可对比性的前提下，当前推荐实验顺序如下：

1. 保持 `MLP + full continuous observation` 作为主基线；
2. 在该基线下继续做 `remove_hinge_status_from_observation=True` 的对照；
3. 增加 `is_critic_bi_head`，验证 phase-aware critic 是否能降低 critic loss 并提升收敛；
4. 在不改 reward 的前提下增加 `is_actor_retentive`，验证历史编码是否能提升铰接效率和纵向跟踪精度；
5. 后续若仍需继续优化 observation，则优先处理：
   - `self_hinge_info` 中的 boundary signal
   - `others_relative_acceleration` 的残余尖峰
...
- **当前更合理的主线是：连续 observation + MLP 基线，再逐步引入 phase-aware critic 和历史编码 actor。**
